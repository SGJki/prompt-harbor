...
| Runtime/client session schema and shared client identity | `tests/test_api_sessions.py`, `tests/test_identity.py`, `tests/test_gateway.py::test_multiple_requests_share_startup_session` |
...
| Full retention cascade, recent chain preservation, runtime/client sessions, idempotence | `tests/test_retention.py::test_purge_removes_full_expired_chain_and_keeps_recent_chain`, `test_purge_repeat_idempotent`, `test_purge_removes_unreferenced_explicit_sessions`, `test_business_cascade_without_fk` |
...
| Real browser UI workflow and explicit session views | `browser-tests/audit.spec.js` via `npm run test:browser`, `tests/test_ui_static.py` |
