...
test('audit desk supports navigation, filters, details, refresh and SSE invalidation', async ({ page }) => {
  ...
  const gateway = spawn(process.env.PYTHON || 'python3', [join(root, 'prompt_harbor.py'), 'start', '--database', database, '--listen', `127.0.0.1:${gatewayPort}`, '--upstream', `http://127.0.0.1:${upstream.address().port}`, '--sse-keepalive', '0.05'], { cwd: root, stdio: ['ignore', 'pipe', 'pipe'] });
  try {
