...
def test_list_does_not_print_body(tmp_path):
 p,_=seeded(tmp_path); r=subprocess.run([sys.executable,'prompt_harbor.py','list','--database',str(p)],capture_output=True,text=True); assert 'body' not in r.stdout and 'abc' not in r.stdout
def test_show_prints_payload(tmp_path):
 p,cid=seeded(tmp_path); r=subprocess.run([sys.executable,'prompt_harbor.py','show',str(cid),'--database',str(p)],capture_output=True,text=True); assert r.returncode==0 and 'abc' in r.stdout and 'body' in r.stdout
def test_show_headers_are_json(tmp_path):
 p,cid=seeded(tmp_path); r=subprocess.run([sys.executable,'prompt_harbor.py','show',str(cid),'--database',str(p)],capture_output=True,text=True); assert '[HEADERS]' in r.stdout and 'X-Test' in r.stdout
